...
@section('header_title', 'Student Profile')
...
@section('content')
    ...
    @endif
                ...
                <p class="text-sm text-gray-500 mt-1">Joined: {{ $student->join_date->format('d M Y') }}</p>

                <div class="mt-4 pt-4 border-t border-gray-100 flex justify-center space-x-4">
                    <a href="{{ route('students.edit', $student) }}" class="inline-flex items-center bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-150">
                         <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        Edit
                    </a>
                    <a href="{{ route('fees.index', ['student_id' => $student->id]) }}" class="inline-flex items-center bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-150">
            ...
            <div class="bg-white rounded-xl shadow-xl p-6 transition duration-300 transform hover:shadow-2xl">
                ...
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <dd class="font-medium text-gray-800">{{ $student->batch->name ?? 'N/A' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500">Class:</dt>
                        <dd class="font-medium text-gray-800">{{ $student->class }}</dd>
                    </div>
                     <div class="flex justify-between">
                        <dt class="text-gray-500">Phone:</dt>
                        <dd class="font-medium text-gray-800 font-mono">{{ $student->phone }}</dd>
                    </div>
                     <div class="flex justify-between">
                        <dt class="text-gray-500">Monthly Fee:</dt>
                        <dd class="font-medium text-gray-800">₹{{ number_format($student->fees, 2) }}</dd>
                    </div>
                </dl>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-xl p-6 transition duration-300 transform hover:shadow-2xl">
                ...
                @if($attendanceSummary->total_days > 0)
                    <div class="flex justify-around items-center text-center">
                        <div>
                            <p class="text-3xl font-bold text-green-600">{{ $attendanceSummary->present_percentage }}<span class="text-xl">%</span></p>
                            <p class="text-sm text-gray-500">Present</p>
                        </div>
                         <div>
                            <p class="text-2xl font-semibold text-green-600">{{ $attendanceSummary->total_present }}</p>
                            <p class="text-xs text-gray-500">Days Present</p>
                        </div>
                        <div>
                            <p class="text-2xl font-semibold text-red-600">{{ $attendanceSummary->total_absent }}</p>
                            <p class="text-xs text-gray-500">Days Absent</p>
                        </div>
                        <div>
                            <p class="text-2xl font-semibold text-gray-600">{{ $attendanceSummary->total_days }}</p>
                            <p class="text-xs text-gray-500">Total Days Marked</p>
                        </div>
                    </div>
                    <div class="mt-4 text-right">
                         <a href="{{ route('attendance.report', ['student_id' => $student->id]) }}" class="text-sm font-medium text-blue-600 hover:underline">View Full Report &rarr;</a>
                    </div>
                @else
            ...
            <div class="bg-white rounded-xl shadow-xl p-6 transition duration-300 transform hover:shadow-2xl">
                 ...
                 ">
                     ...
                     <p class="text-3xl font-bold {{ $balanceColor }}">
                         ...
                         <span class="text-xl font-medium">{{ $currentBalance > 0 ? 'Due' : ($currentBalance < 0 ? 'Credit' : 'Settled') }}</span>
                     </p>
                 </div>
                     ...
                     <table class="min-w-full leading-normal">
                         ...
                         <tbody class="text-gray-700 text-sm">
                             @forelse ($recentTransactions as $tx)
                                 <tr class="border-t border-gray-100 {{ $loop->even ? 'bg-gray-50/50' : '' }}">
                                     <td class="py-2 px-3 text-left">{{ $tx->date->format('d-M-y') }}</td>
                                     <td class="py-2 px-3 text-left">{{ $tx->description }}</td>
                                     <td class="py-2 px-3 text-right font-semibold {{ $tx->amount > 0 ? 'text-green-600' : 'text-red-600' }}">
                 ...
                 <div class="mt-4 text-right">
                      <a href="{{ route('fees.index', ['student_id' => $student->id]) }}" class="text-sm font-medium text-blue-600 hover:underline">View Full Ledger &rarr;</a>
                 </div>
            </div>
