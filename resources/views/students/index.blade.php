...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4"> {{-- Increased bottom margin --}}
        <a href="{{ route('students.create') }}" class="w-full md:w-auto btn-start text-white font-bold py-2 px-4 rounded-lg shadow hover:bg-green-700 transition duration-300 text-center flex items-center justify-center space-x-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            <span>Add New Student</span>
        </a>

        <!-- Filter Form -->
        <form method="GET" action="{{ route('students.index') }}" class="w-full md:w-auto flex flex-col md:flex-row items-center gap-3">
            <select name="batch_filter" class="block w-full md:w-48 bg-white border border-gray-300 rounded-lg py-2 px-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-700"> {{-- Updated focus and text color --}}
            ...
            <button type="submit" class="w-full md:w-auto bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow hover:bg-blue-700 transition duration-300 flex items-center justify-center space-x-2"> {{-- Changed filter button to blue --}}
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200"> {{-- Added subtle border --}}
        <div class="overflow-x-auto">
            <table class="min-w-full leading-normal">
                <thead>
                    <tr class="table-header"> {{-- Applied custom table header style --}}
                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse ($students as $student)
                        <tr class="border-b border-gray-100 table-row-hover" {{-- Lighter border, custom hover --}}
                                ...
                                <a href="{{ route('students.profile', $student) }}" 
                                   ...
                                   class="text-purple-600 hover:text-purple-900 font-semibold">
                                    ...
                                    <a href="{{ route('fees.index', ['student_id' => $student->id]) }}"
                                       onclick="event.stopPropagation()"
                                       class="action-button pay-fee"> {{-- Applied custom action button style --}}
                                        Pay Fee
                                    </a>
                                    <a href="{{ route('students.edit', $student) }}"
                                       onclick="event.stopPropagation()"
                                       class="action-button edit"> {{-- Applied custom action button style --}}
                                        Edit
                                    </a>
                                    <form action="{{ route('students.destroy', $student) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="action-button delete"> {{-- Applied custom action button style --}}
                                            Delete
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-6 px-4 text-center text-gray-500 text-base">No students found. Add one to get started!</td> {{-- Larger text for empty state --}}
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="p-4 bg-white pagination-container"> {{-- Added custom class for pagination --}}
