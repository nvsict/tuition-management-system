    ...
    <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <form method="GET" action="{{ route('attendance.report') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="student_id" id="student_id" class="block w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                    ...
                    @foreach($allStudents as $student)
                        <option value="{{ $student->id }}" {{ $filters['selectedStudent'] == $student->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="date" name="start_date" id="start_date" value="{{ $filters['startDate'] }}" class="block w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
            </div>

            <!-- End Date -->
            <div>
                <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date</label>
                <input type="date" name="end_date" id="end_date" value="{{ $filters['endDate'] }}" class="block w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
            </div>
            ...
            <div class="col-span-1 md:col-span-2 lg:col-span-4 flex justify-end gap-4 mt-4">
                ...
                <a href="{{ route('attendance.report') }}" class="btn-stop text-white font-bold py-2 px-6 rounded-lg shadow hover:bg-orange-700 transition duration-300">
                    Clear Filters
                </a>
                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse ($reportData as $data)
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            <td class="py-3 px-4 md:px-6 text-left whitespace-nowrap font-semibold">{{ $data->name }}</td>
                            <td class="py-3 px-4 md:px-6 text-left">{{ $data->batch_name }}</td> <!-- ADDED -->
